...
import ordersFeedItemStyles from './orders-feed-item.module.css';
...
const OrdersFeedItem = ({ createdAt, ingredients, name, number, status, updatedAt, _id, displayStatus }: IOrdersFeedItem) => {
  ...
  return (
    <div className={`${ordersFeedItemStyles.ordersFeedItemContainer} p-6`}>
      <div className={ordersFeedItemStyles.ordersFeedItemHeading}>
      ...
      <div className={ordersFeedItemStyles.ordersFeedItemDetails}>
        <div className={ordersFeedItemStyles.ordersFeedItemIngredients}>

          {previewIngredients.map((item, index) => (
            <div className={ordersFeedItemStyles.ordersFeedIngredientPreview} key={number+item._id+index}>
              <img src={item.image} className={ordersFeedItemStyles.ordersFeedIngredientPreviewImage} alt={item.name} />
              {index === 5 &&
                <div className={`${ordersFeedItemStyles.ordersFeedRestIngredients} text text_type_main-default`}>{`+${restIngredients}`}</div>
              }
            </div>
          ))}

        </div>
        <div className={ordersFeedItemStyles.ordersFeedItemCost}>
